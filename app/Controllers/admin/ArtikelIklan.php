...
class ArtikelIklan extends BaseController
{
    ...
    private $HargaIklan;
    private $OlehOleh;
    private $TempatWisata;

    public function __construct()
    {
        $this->ArtikelIklanModel = new ArtikelIklanModel();
        $this->Artikel = new ArtikelModel();
        $this->OlehOleh = new OlehOlehModel();
        $this->TempatWisata = new TempatWisataModel();
        $this->HargaIklan = new HargaIklanModel();
    }
    ...
    public function index()
    {
        $all_data_artikeliklan = $this->ArtikelIklanModel->getArtikelIklanAnjay();
        $artikelModel = new \App\Models\ArtikelModel();
        $olehOlehModel = new \App\Models\OlehOlehModel();
        $tempatWisataModel = new \App\Models\TempatWisataModel();

        foreach ($all_data_artikeliklan as &$iklan) {
            $judul = 'Tidak ditemukan';

            if ($iklan['tipe_content'] === 'artikel') {
                $data = $artikelModel->find($iklan['id_content']);
                $judul = $data->judul_artikel ?? 'Tidak ditemukan';
            } elseif ($iklan['tipe_content'] === 'oleholeh') {
                $data = $olehOlehModel->find($iklan['id_content']);
                $judul = $data['nama_oleholeh'] ?? 'Tidak ditemukan';
            } elseif ($iklan['tipe_content'] === 'tempatwisata') {
                $data = $tempatWisataModel->find($iklan['id_content']);
                $judul = $data['nama_wisata_ind'] ?? 'Tidak ditemukan';
            }

            $iklan['judul_content'] = $judul;
        }
    ...
    public function ubahStatus()
    {
        ...
        $tipe_content = $this->request->getPost('tipe_content');

        if ($tipe_content === 'artikel') {
            $this->Artikel->update($id_artikel, [
                $kolomIklan => $status === 'diterima' ? 'ada' : 'tidak'
            ]);
        } elseif ($tipe_content === 'oleholeh') {
            $this->OlehOleh->update($id_artikel, [
                $kolomIklan => $status === 'diterima' ? 'ada' : 'tidak'
            ]);
        } elseif ($tipe_content === 'tempatwisata') {
            $this->TempatWisata->update($id_artikel, [
                $kolomIklan => $status === 'diterima' ? 'ada' : 'tidak'
            ]);
        }
