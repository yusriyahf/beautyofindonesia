...
<div class="container">
    <div class="row mt-3">
        ...
        <div class="col-12 mt-5">
            <div class="section-title">
                <h1 id="sectionTitle" class="m-0 text-uppercase font-weight-bold h4"><?= esc($title); ?></h1>
            </div>
        </div>

        <!-- Filter form -->
        <div class="col-12 mb-4">
            <form method="get" action="<?= base_url($lang === 'en' ? 'en/destinations/' . $tempatwisata->slug_kategori_wisata_en : 'id/wisata/' . $tempatwisata->slug_kategori_wisata) ?>">
                <div class="row">
                    <!-- Provinsi Filter -->
        ...
        <script>
            $(document).ready(function() {
                const lang = "<?= $lang; ?>";
                const kategoriWisata = "<?= $lang === 'id' ? $tempatwisata->nama_kategori_wisata : $tempatwisata->nama_kategori_wisata_en; ?>";
                ...
                function updateSectionTitle(provinsiName, kabupatenName) {
                    ...
                    if (kabupatenName && kabupatenName !== "" && kabupatenName !== "<?= lang('Blog.btnAllKabupaten') ?>") {
                        title = lang === 'id' ? `${kategoriWisata} di ${kabupatenName}` :
                            `${kategoriWisata} in ${kabupatenName}`;

                    } else if (provinsiName && provinsiName !== "") {
                        title = title = lang === 'id' ? `${kategoriWisata} di ${provinsiName}` :
                            `${kategoriWisata} in ${provinsiName}`;
                    }
        ...
        <?php if (!empty($wisataList) && is_array($wisataList)) : ?>
    ...
    <div class="pagination-container mb-4">
        <?= $pager->links('wisata', 'default_pagination') ?>
    </div>
</div>
